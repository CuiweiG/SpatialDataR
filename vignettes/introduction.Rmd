...
SpatialData-formatted Zarr stores (Marconato et al. 2024
...
- **No Python required**: reads `.zarr` stores using R packages
  (`Rarr`, `pizzarr`, `arrow`, `jsonlite`)
- **SpatialData-aware**: parses `.zattrs` metadata, discovers
  elements, handles coordinate systems and transforms
- **Bioconductor-native**: returns `DataFrame`, `SimpleList`,
  and `SpatialExperiment` objects
- **OME-NGFF compliant**: supports the full coordinate
  transformation vocabulary (identity, scale, translation,
  affine, sequence)
...
- Moore J et al. (2023). OME-Zarr: a cloud-optimized bioimaging
  file format with international community support. *Histochem
  Cell Biol* 160:223-251. doi:10.1007/s00418-023-02209-1

- Righelli D et al. (2022). SpatialExperiment: infrastructure
  for spatially-resolved transcriptomics data in R using
  Bioconductor. *Bioinformatics* 38:3128-3131.
